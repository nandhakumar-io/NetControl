...
import {
  ...
  FileText, Shield, RefreshCw, Monitor, X
} from 'lucide-react'
...
function DropZone({ file, onFile }) {
  ...
  const onDrop = (e) => { e.preventDefault(); setOver(false); handle(e.dataTransfer.files[0]) }
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className="relative flex flex-col items-center justify-center gap-2 h-36 rounded-xl border-2 border-dashed cursor-pointer transition-all duration-200"
      style={over
        ? { borderColor: 'rgba(14,165,233,0.6)', background: 'rgba(14,165,233,0.06)' }
        : file
          ? { borderColor: 'rgba(34,197,94,0.4)', background: 'rgba(34,197,94,0.05)' }
      ...
      {file ? (
        ...
        <>
          ...
          <p className="text-sm font-body" style={{ color: 'var(--text-secondary)' }}>
            Drop a file here or <span style={{ color: '#38bdf8' }}>browse</span>
          </p>
...
function DeviceCheckbox({ device, checked, onChange }) {
  ...
  return (
    <button
      ...
      style={checked
        ? { borderColor: 'rgba(14,165,233,0.4)', background: 'rgba(14,165,233,0.08)' }
        : { borderColor: 'var(--border-subtle)', background: 'var(--bg-input)' }
      }
    >
      {/* Checkbox square */}
      <div
        className="w-4 h-4 rounded flex items-center justify-center shrink-0 transition-all border-2"
        style={checked
...
function ResultRow({ r }) {
  ...
  return (
    <div className="flex items-start gap-3 py-3 border-b last:border-b-0" style={{ borderColor: 'var(--border-subtle)' }}>
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-body font-medium" style={{ color: 'var(--text-primary)' }}>{r.device}</p>
        <p className="text-xs font-mono mt-0.5 break-all" style={{ color: ok ? 'var(--text-muted)' : '#ef4444' }}>{r.details}</p>
      </div>
...
export default function FilePushPage() {
  ...
  const filteredDevices = devices.filter(d =>
    d.name.toLowerCase().includes(searchQ.toLowerCase()) || d.ip_address.includes(searchQ)
  )

  const resolvedIds = targetMode === 'devices' ? [...selectedIds]
    : targetMode === 'all' ? devices.map(d => d.id) : []

  const targetLabel = targetMode === 'group'
    ? (groups.find(g => g.id === groupId)?.name ? `All devices in "${groups.find(g => g.id === groupId).name}"` : '—')
    : targetMode === 'all' ? `All ${devices.length} devices`
    : `${resolvedIds.length} device${resolvedIds.length !== 1 ? 's' : ''} selected`

  const canSubmit = file && remotePath.trim() && pin.trim() &&
  ...
  const handlePush = async () => {
    ...
    else form.append('deviceIds', JSON.stringify(resolvedIds))
    try {
  ...
  const handleReset = () => {
    ...
    setTargetMode('devices'); setGroupId(''); setSelectedIds(new Set())
    setPin(''); setResult(null)
  }
      ...
      {result && (
        <div className="card space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div className="ml-auto flex gap-4">
              <div className="text-center">
                ...
                <p className="text-xs" style={{ color: 'var(--text-muted)' }}>succeeded</p>
              </div>
              <div className="text-center">
          ...
          <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border-subtle)' }}>
            <div className="px-4 py-2.5" style={{ background: 'var(--bg-input)' }}>
              <p className="text-xs font-body font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Device Results</p>
            </div>
            <div className="px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-5">
            <div className="card space-y-5">
              <h2 className="text-xs font-body font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>File</h2>
              <Field label="File to push">
                <DropZone file={file} onFile={setFile} />
              </Field>
              <Field label="Remote destination path" hint="Full path including filename on the remote device">
                <input className="input-field" placeholder="/tmp/script.sh"
                  value={remotePath} onChange={e => setRemotePath(e.target.value)} />
              </Field>
              <Field label="File permissions">
                <div className="flex gap-1.5 flex-wrap">
                  {[['0644','0644 (Read-only)'],['0755','0755 (Executable)'],['0600','0600 (Private)']].map(([val, lbl]) => (
                    <button key={val} onClick={() => setFileMode(val)}
                      className={`chip ${fileMode === val ? 'chip-selected' : ''}`}>{lbl}</button>
                  ))}
                </div>
              </Field>
            </div>
            ...
            <div className="card space-y-4">
              <h2 className="text-xs font-body font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>Authorise</h2>
              <Field label="Action PIN">
                <div className="relative">
                  ...
                  <input type="password" className="input-field pl-8" placeholder="Enter your action PIN"
                    ...
                    onKeyDown={e => e.key === 'Enter' && canSubmit && handlePush()} autoComplete="off" />
                </div>
              </Field>
              ...
              <div className="px-3 py-2.5 rounded-lg text-xs font-body space-y-1"
                ...
                <p style={{ color: 'var(--text-secondary)' }}>
                  {file
                    ? <span className="font-medium" style={{ color: '#38bdf8' }}>{file.name}</span>
              ...
              <button onClick={handlePush} disabled={!canSubmit || pushing}
          ...
          <div className="lg:col-span-2 card space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <h2 className="text-xs font-body font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>Target Devices</h2>
              {/* Mode tabs */}
              <div className="flex gap-1 p-1 rounded-lg" style={{ background: 'var(--bg-input)', border: '1px solid var(--border-subtle)' }}>
                {[['devices','Select devices'],['group','By group'],['all',`All (${devices.length})`]].map(([val, lbl]) => (
                  <button key={val} onClick={() => setTargetMode(val)}
                    className="px-3 py-1.5 rounded-md text-xs font-body font-medium transition-all"
                    style={targetMode === val
              ...
              <>
                <div className="flex items-center gap-2">
                  <div className="relative flex-1">
                    <Monitor size={13} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-muted)' }} />
                    <input className="input-field pl-8 py-2 text-sm" placeholder="Search devices…"
                  ...
                  <button
                    ...
                    {selectedIds.size === filteredDevices.length && filteredDevices.length > 0 ? 'Deselect all' : 'Select all'}
                  </button>
                </div>
                ...
                {loading ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-96 overflow-y-auto pr-1">
                    {filteredDevices.map(d => (
                      <DeviceCheckbox key={d.id} device={d} checked={selectedIds.has(d.id)} onChange={toggleDevice} />
                    ))}
                  </div>
                )}
